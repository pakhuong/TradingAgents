...
- **Macro news**: Cross-market or economy-wide headlines that are not tied to one issuer. In the adapter, this is enforced by deterministic filtering over sponsor news metadata such as title, description, short description, category, tags, and content when present.
...
- **REQ-007**: `get_global_news(curr_date, look_back_days, limit)` shall prefer `vnstock_news` in the first sponsor-aware pass because it is the sponsor module whose documented purpose matches multi-source market news aggregation, and shall apply adapter-local deterministic relevance filtering before the final sort/limit step.
...
| New global-news helper, if added                            | `vnstock_news`                                                                         | `list_supported_sites`, `Crawler`, `AsyncBatchCrawler` or equivalent documented surface   | current clear no-data message                        | Keep this helper isolated so crawler complexity does not leak into company-data helpers, and keep deterministic macro filtering local to the adapter. |
...
| `get_global_news(curr_date, look_back_days, limit)`           | `vnstock_news`         | crawler or site-list-based market news retrieval | current clear unavailable message                        | This is the only current router-facing function that should directly target the sponsor news module in the first pass, with deterministic macro filtering over RSS/article metadata before sorting and truncation. |
...
- **AC-006**: Given `vnstock_news` is installed, when `get_global_news(...)` is called, then the adapter uses the sponsor news module, filters the results to macro-relevant or market-wide items, and avoids returning issuer-scoped finance stories that lack macro signals.
...
- **AC-011**: Given `vnstock_news` returns RSS-style metadata with fields such as `description` and `pubDate`, when `get_global_news(...)` is called, then the adapter still date-filters and evaluates relevance correctly.
- **AC-012**: Given recent `vnstock_news` items are all issuer-specific, lifestyle, or otherwise non-macro, when `get_global_news(...)` is called, then the adapter returns the current clear unavailable message rather than unrelated headlines.
...
- **Coverage Focus**:
  ...
  - TA preference and local fallback for `get_indicator`.
    ...
    - Deterministic macro filtering for sponsor news, including RSS-style metadata aliases.
...
- `test_get_global_news_uses_rss_style_metadata_for_macro_filter()`
- `test_get_global_news_accepts_macro_indicator_terms()`
- `test_get_global_news_excludes_issuer_story_despite_market_section_labels()`
- `test_get_global_news_recognizes_pubdate_field()`
...
# Preferred: vnstock_news crawler/site aggregation filtered to macro-relevant items
# Fallback: current clear unavailable message if sponsor news is absent or all fetched items are filtered as non-macro
```
